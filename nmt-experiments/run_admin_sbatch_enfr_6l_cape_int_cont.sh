...
fairseq-train ../data-bin/wmt14_en_fr_joined_dict \
	...
	--seed 1111 --restore-file checkpoint_last.pt --max-epoch 75 --save-dir wmt14enfr/wmt-admin-cape-6l-${gl}-int \
	--user-dir ../radam_fairseq --init-type adaptive \
  	--profiling_path wmt14enfr/wmt-admin-cape-6l-${gl}-int/profile.ratio.init \
	--french --augmented-embedding --global-shift=${gl} --global-shift-int \
	--log-format simple --log-interval 100 | tee ./wmt14enfr/log/loss-admin-cape-6l-${gl}-int-cont.log
