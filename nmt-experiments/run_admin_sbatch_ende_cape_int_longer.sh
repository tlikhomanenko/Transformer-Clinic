...
CUDA_VISIBLE_DEVICES=$GPUID fairseq-train \
  ...
  --lr-scheduler inverse_sqrt --max-update 750000 \
  ...
  --save-dir wmt14ende/wmt-admin-cape-${lnum}l-gl-${gl}-int-750k --restore-file x.pt --seed 1111 \
  --user-dir ../radam_fairseq --log-format simple --log-interval 500 \
  --init-type adaptive-profiling --profiling_path wmt14ende/wmt-admin-cape-${lnum}l-gl-${gl}-int-750k/profile.ratio.init \
  --fp16 --fp16-scale-window 256 \
  --encoder-layers $lnum --decoder-layers $lnum --augmented-embedding --global-shift=${gl} --global-shift-int \
  --threshold-loss-scale 0.03125 | tee ./wmt14ende/log/loss_admin-cape-${lnum}l-gl-${gl}-int-750k.log
...
fairseq-train \
  ...
  --save-dir wmt14ende/wmt-admin-cape-${lnum}l-gl-${gl}-int-750k --restore-file x.pt --seed 1111 \
  --user-dir ../radam_fairseq --log-format simple --log-interval 500 \
  --init-type adaptive --profiling_path wmt14ende/wmt-admin-cape-${lnum}l-gl-${gl}-int-750k/profile.ratio.init \
  --fp16 --fp16-scale-window 256 \
  --encoder-layers $lnum --decoder-layers $lnum --augmented-embedding --global-shift=${gl} --global-shift-int \
  --threshold-loss-scale 0.03125 | tee ./wmt14ende/log/loss_admin-cape-${lnum}l-gl-${gl}-int-750k.log


bash eval_wmt_en-de.sh wmt14ende/wmt-admin-cape-${lnum}l-gl-${gl}-int-750k $GPUID 
