...
CUDA_VISIBLE_DEVICES=$GPUID fairseq-train \
  ...
  --save-dir wmt14ende/wmt-admin-abspos-${lnum}l --restore-file x.pt --seed 1111 \
  --user-dir ../radam_fairseq --log-format simple --log-interval 500 \
  --init-type adaptive-profiling --profiling_path wmt14ende/wmt-admin-abspos-${lnum}l/profile.ratio.init \
...
fairseq-train \
  ...
  --save-dir wmt14ende/wmt-admin-abspos-${lnum}l --restore-file x.pt --seed 1111 \
  --user-dir ../radam_fairseq --log-format simple --log-interval 500 \
  --init-type adaptive --profiling_path wmt14ende/wmt-admin-abspos-${lnum}l/profile.ratio.init \
  --fp16 --fp16-scale-window 256 \
  --encoder-layers $lnum --decoder-layers $lnum \
  --encoder-learned-pos --decoder-learned-pos \
  --threshold-loss-scale 0.03125 | tee ./wmt14ende/log/loss_admin-abspos-${lnum}l.log

bash eval_wmt_en-de.sh wmt14ende/wmt-admin-abspos-${lnum}l $GPUID 
